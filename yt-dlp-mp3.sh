...
for item in "${items[@]}"; do
    ...
    echo "Try to download as mp3: ${item} with yt-dlp"

    if command -v yt-dlp >/dev/null 2>&1; then
        yt-dlp --ignore-errors --format bestaudio \
        --extract-audio --audio-format mp3 --audio-quality 160K --add-metadata --embed-thumbnail --force-overwrite \
        --output '%(title)s.%(ext)s' --paths "$OUTDIR" "$item"

        if [ $? -ne 0 ]; then
           echo "Error Code: $? - Downloading as mp3: $item yt-dlp"
           echo "Error Code: $? - Downloading as mp3: $item yt-dlp" >> $SCRIPT_PATH/yt-dlp-error.log
        else
           echo "Download Succeed as mp3: $item yt-dlp" >> $SCRIPT_PATH/yt-dlp-url_succeed.log
        fi

    else
        cd "$SCRIPT_PATH"
        python -m yt-dlp --ignore-errors --format bestaudio \
        --extract-audio --audio-format mp3 --audio-quality 160K --add-metadata --embed-thumbnail --force-overwrite \
        --output '%(title)s.%(ext)s' --paths "$OUTDIR" "$item"
