...
for item in "${items[@]}"; do
    ...
    if ! $IGNORE_EXISTING; then
        # Check if both item and "720p" exist in the log
        if grep -q "${item}" "$SCRIPT_PATH/yt-dlp-url_succeed.log" && grep -q "720p" "$SCRIPT_PATH/yt-dlp-url_succeed.log"; then
            echo "File already downloaded: ${item} 720p, skipping..."
            continue
        fi
    fi
    ...
    if command -v yt-dlp >/dev/null 2>&1; then
        ...
        cd "$SCRIPT_PATH"
        python -m yt-dlp --ignore-errors -f "bestvideo[height<=720]+bestaudio/best[height<=720]" --add-metadata --embed-thumbnail --force-overwrite --merge-output-format mp4 \
        --output "%(title)s.%(ext)s" --paths "$OUTDIR" "$item"'
